(() => {
  ...
  const dataUrl = new URL('../data/ambient-map-shapes.json?v=20260819-5shapes', currentScript.src);
  const primitivesUrl = new URL('ambient-background-primitives.js?v=20260819-5shapes', currentScript.src);
  ...
  function loadScriptOnce(src) {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.src = src;
      script.async = false;
      script.dataset.ambientPrimitives = 'true';
  ...
  async function loadRoadLayers() {
    ...
    if (!response.ok) throw new Error(`Ambient road metadata returned ${response.status}`);
    const payload = await response.json();
    const layers = [];
    for (const layer of payload.layers || []) {
      if (!layer.id || !layer.file) continue;
      const url = new URL(`../data/${layer.file}?v=20260819-5shapes`, currentScript.src);
      try {
        const imageResponse = await fetch(url, { cache: 'force-cache' });
        if (!imageResponse.ok) throw new Error(`${layer.file} returned ${imageResponse.status}`);
        ...
        // A failed layer is skipped without disabling the rest of the ambient scene.
      }
    }
    return layers;
  }
  ...
  async function start() {
    ...
    try {
      await loadScriptOnce(primitivesUrl.href);
    } catch {
      return;
    }
    ...
    let roadLayers = [];
    try { roadLayers = await loadRoadLayers(); } catch { roadLayers = []; }
    ...
    const drawPrimitive = (ctx, item, x, y, opacity) => {
      ...
      const layer = roadMap.get(item.roadLayer);
    ...
    function setDurations(item, q) {
      if (item.type === 'road') {
        item.fadeInDuration = 2800 + q() * 1400;
        item.holdDuration = 12000 + q() * 9000;
        item.fadeOutDuration = 3600 + q() * 1800;
        item.hiddenDuration = 800 + q() * 1800;
      } else {
    ...
    function pointInsideCartographySlot(x, y, margin = 0) {
      const nx = x / Math.max(viewportWidth, 1);
      const ny = y / Math.max(viewportHeight, 1);
      return CARTOGRAPHY_SLOTS.some((slot) =>
        nx > slot.x - margin && nx < slot.x + slot.w + margin &&
        ny > slot.y - margin && ny < slot.y + slot.h + margin
      );
    }
    ...
    function chooseViewportPosition(item, q) {
      if (item.cartography) {
        ...
        const insetX = slot.w * (0.14 + q() * 0.12);
        const insetY = slot.h * (0.14 + q() * 0.12);
        item.x = (slot.x + insetX + q() * Math.max(0.01, slot.w - insetX * 2)) * viewportWidth;
        item.y = (slot.y + insetY + q() * Math.max(0.01, slot.h - insetY * 2)) * viewportHeight;
        const slotPixels = Math.min(slot.w * viewportWidth, slot.h * viewportHeight);
        item.size = clamp(slotPixels * (1.0 + q() * 0.42), mobileViewport.matches ? 108 : 145, mobileViewport.matches ? 190 : 285);
        return;
      }
      let x = q() * viewportWidth;
      let y = q() * viewportHeight;
      let attempts = 0;
      while (pointInsideCartographySlot(x, y, 0.015) && attempts < 24) {
        x = q() * viewportWidth;
        y = q() * viewportHeight;
        attempts += 1;
      }
    ...
    function respawnItem(item) {
      ...
      configureItem(item, q, item.cartography ? 'road' : null);
    ...
    function primeLifecycle(item, now, q) {
      ...
      const phase = q();
      item.state = phase < .08 ? 'hidden' : phase < .18 ? 'fadeIn' : phase < .86 ? 'hold' : 'fadeOut';
      item.stateStartedAt = now - q() * item[`${item.state}Duration`];
    }

    function buildWorld() {
      const normalDensity = shouldReduceMotion() ? (mobileViewport.matches ? 3 : 7) : (mobileViewport.matches ? 5 : 12);
      ...
      items = [];
      for (let index = 0; index < normalDensity; index += 1) {
    ...
    function advanceItem(item, dt) {
      ...
      if (pointInsideCartographySlot(item.x, item.y, 0.005)) {
    ...
    function draw(now, dt) {
      ...
      const sweepX = Math.sin(now * .000025) * (mobileViewport.matches ? 8 : 18) * motion;
      const sweepY = Math.cos(now * .000021) * (mobileViewport.matches ? 6 : 14) * motion;
      for (const item of items) {
        advanceItem(item, dt);
        const life = stateOpacity(item, now); if (life < .015) continue;
